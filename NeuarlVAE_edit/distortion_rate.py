...
# --- Patch: ensure torch.distributions parameters live on the same device as 'device'
# MixtureSameFamily/Normal/Categorical often carry CPU tensors even when models are on CUDA.
def _move_distribution_to_device(px, device):
    try:
        import torch
        from torch.distributions import MixtureSameFamily, Normal, Categorical

        if isinstance(px, MixtureSameFamily):
            mix = px.mixture_distribution
            comp = px.component_distribution

            # mixture params
            logits = getattr(mix, "logits", None)
            probs  = getattr(mix, "probs", None)

            logits = logits.to(device) if logits is not None else None
            probs  = probs.to(device) if probs is not None else None

            mix2 = Categorical(logits=logits) if logits is not None else Categorical(probs=probs)

            # component params
            loc = comp.loc.to(device)
            scale = comp.scale.to(device)
            comp2 = Normal(loc, scale)

            return MixtureSameFamily(mix2, comp2)

        # If it's a Normal (or other) distribution, handle common attributes
        if hasattr(px, "loc") and hasattr(px, "scale"):
            return type(px)(px.loc.to(device), px.scale.to(device))
...
def vary_R(RtVec,x_test):
    ...
    x_samples = p_x = _move_distribution_to_device(p_x, device)
    p_x.sample((N_SAMPLES,))[:,None]
...
for i in range(7):
    ...
    w = torch.distributions.Categorical(torch.tensor([0.3,0.2,0.5], device=device))
    gs = torch.distributions.normal.Normal(torch.Tensor([-4,0,2]),torch.tensor([1,0.5,1], device=device))
    p_x = torch.distributions.mixture_same_family.MixtureSameFamily(w,gs)
    p_x = _move_distribution_to_device(p_x, device)
